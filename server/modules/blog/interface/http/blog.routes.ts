...
const serviceSchema = z
  .object({
    ...
    href: requiredText(1000).refine(
      isSafeBlogNavigationUrl,
      "Use a relative or http(s) service link.",
    ),
...
const postSchema = z
  .object({
    ...
    sidebarVideoUrl: z.preprocess(
      blankToNull,
      text(500).url().nullable().optional(),
    ),
    sidebarVideoTitle: z.preprocess(
      blankToNull,
    ...
    translations: z
      ...
      .max(2)
      .refine(
        (items) => items.some((item) => item.locale === "en"),
        "Keep an English version for the public article.",
      ),
...
const updateSchema = z
  .object({ expectedRevision: z.number().int().positive(), post: postSchema })
  .strict();
...
const restoreSchema = z
  ...
  .strict();
const orderSchema = z
  .object({ ids: z.array(z.string().trim().min(1).max(191)).min(1).max(500) })
  .strict();
...
function normalizeInput(input: z.infer<typeof postSchema>): BlogPostInput {
  ...
  const english =
    ...
    translations[0];
  const explicitPrimaryIndex = input.services.findIndex(
  ...
  const videoId = input.sidebarVideoUrl
    ? getYouTubeVideoId(input.sidebarVideoUrl)
    : null;
  if (input.sidebarVideoUrl && !videoId)
    throw new Error(
  ...
  return {
    ...
    readTimeMinutes:
      input.readTimeMinutes || estimateReadTime(english?.bodyHtml ?? ""),
...
function sendKnownError(error: unknown, reply: FastifyReply) {
  if (
    ...
    error instanceof BlogSafetyError ||
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    if (error.code === "P2002")
      return reply
        .code(409)
        .send({ error: "An article with this slug already exists." });
    if (error.code === "P2025")
  ...
  if (error instanceof z.ZodError)
    return reply
      .code(400)
      .send({
        error: error.issues[0]?.message ?? "Check the article details.",
      });
  if (
    error instanceof Error &&
    error.message.startsWith("Paste a valid YouTube")
  )
    return reply.code(400).send({ error: error.message });
  throw error;
}

type ParsedBlogImage = {
  image: ImageUpload;
  originalName: string;
  width?: number;
  height?: number;
...
async function readBlogImage(
  request: FastifyRequest,
): Promise<ParsedBlogImage> {
  if (!request.isMultipart())
    throw new BlogInputError("Choose an image to upload.");
  ...
  for await (const part of request.parts()) {
    if (part.type === "file") {
      ...
      if (!isSupportedBlogImageType(part.mimetype))
        throw new BlogInputError("Use a JPG, PNG or WebP image.");
      if (!body.byteLength)
        throw new BlogInputError("The selected image is empty.");
      if (body.byteLength > MAX_BLOG_IMAGE_BYTES)
        throw new BlogInputError("The image must be 5 MB or smaller.");
    ...
    if (part.fieldname === "altText")
      altText = String(part.value).trim().slice(0, 240);
    if (part.fieldname === "caption")
      caption = String(part.value).trim().slice(0, 300);
  }
...
export function createBlogRoutes(
  ...
  return async function blogRoutes(app: FastifyInstance) {
    app.get("/api/blog/posts", async (request, reply) => {
      ...
      return {
        data: await blogService.getPublicIndex({
          ...input,
    ...
    app.get("/api/blog/media/:id", async (request, reply) => {
      ...
      if (media.post.status !== "PUBLISHED" && !getAdminSession(request))
        return reply.code(404).send({ error: "Image not found." });
      ...
      try {
        ...
        console.warn(
          `Direct streaming for blog media ${id} failed, falling back to signed URL:`,
          streamError,
        );
      }
    ...
    app.get("/api/admin/blog/posts", async (request, reply) => {
      ...
      return {
        data: await blogService.getAdminIndex({
          ...input,
          query: input.query ?? undefined,
          page: input.page,
    ...
    app.get("/api/admin/blog/slug-availability", async (request, reply) => {
      ...
      const input = z
        .object({
          slug: requiredText(180),
          excludeId: z.preprocess(blankToNull, text(191).nullable().optional()),
        })
        .parse(request.query);
    ...
    app.post("/api/admin/blog/posts", async (request, reply) => {
      ...
      try {
        ...
        return reply
          .code(201)
          .send({
            data: await blogService.createPost(input, session.username),
          });
      } catch (error) {
        return sendKnownError(error, reply);
      }
    ...
    app.post("/api/admin/blog/posts/:id/publish", async (request, reply) => {
      ...
      try {
        ...
        return {
          data: await blogService.publishPost(
            id,
            expectedRevision,
            session.username,
          ),
    ...
    app.post("/api/admin/blog/posts/:id/unpublish", async (request, reply) => {
      ...
      try {
        ...
        const { expectedRevision } = revisionSchema.parse(request.body);
        return {
          data: await blogService.unpublishPost(
            id,
            expectedRevision,
            session.username,
          ),
    ...
    app.get("/api/admin/blog/posts/:id/revisions", async (request, reply) => {
      ...
      try {
        const { id } = idSchema.parse(request.params);
        const rows = await prisma.blogPostRevision.findMany({
          where: { postId: id },
          select: {
            id: true,
            version: true,
    ...
    app.post("/api/admin/blog/posts/:id/restore", async (request, reply) => {
      ...
      try {
        ...
        if (!revision)
          throw new BlogNotFoundError("That revision is no longer available.");
    ...
    app.delete("/api/admin/blog/media/:id", async (request, reply) => {
      ...
      try {
        ...
        const post = await prisma.blogPost.findUnique({
          ...
          select: { coverMediaId: true, publishedSnapshot: true },
        });
        const publishedSnapshot =
          post?.publishedSnapshot &&
          typeof post.publishedSnapshot === "object" &&
          !Array.isArray(post.publishedSnapshot)
            ? (post.publishedSnapshot as Record<string, unknown>)
            : null;
        if (post?.coverMediaId === id || publishedSnapshot?.coverMediaId === id)
          return reply
            .code(400)
            .send({
              error: "Choose another cover image before removing this one.",
            });
