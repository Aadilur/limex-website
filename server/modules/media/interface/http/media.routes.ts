...
async function readMediaUpload(request: FastifyRequest): Promise<ParsedMediaUpload> {
  if (!request.isMultipart()) throw new MediaInputError("Choose an image to upload.");
  ...
  for await (const part of request.parts()) {
    if (part.type === "file") {
      ...
      if (!isSupportedBlogImageType(part.mimetype)) throw new MediaInputError("Use a JPG, PNG or WebP image.");
      if (!body.byteLength) throw new MediaInputError("The selected image is empty.");
      if (body.byteLength > MAX_BLOG_IMAGE_BYTES) throw new MediaInputError("The image must be 5 MB or smaller.");
    ...
    if (part.fieldname === "displayName") displayName = String(part.value).trim();
  ...
  return { folderId, image, originalName, displayName, width, height, altText, caption };
}

function sendKnownError(error: unknown, reply: FastifyReply) {
  if (error instanceof MediaConflictError || error instanceof MediaInUseError || error instanceof MediaInputError || error instanceof MediaNotFoundError) {
    return reply.code(error.statusCode).send({ error: error.message });
  }
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    if (error.code === "P2002") return reply.code(409).send({ error: "That media item already exists." });
    if (error.code === "P2025") return reply.code(404).send({ error: "Media was not found." });
  }
  if (error instanceof z.ZodError) return reply.code(400).send({ error: error.issues[0]?.message ?? "Check the media details." });
  throw error;
}

export function mediaRoutes(app: FastifyInstance, options: { service: MediaService }) {
  ...
  app.get("/api/admin/media", async (request, reply) => {
    ...
    try {
      const input = z.object({ folderId: z.preprocess(blankToNull, z.string().trim().min(1).max(191).nullable().optional()) }).parse(request.query);
  ...
  app.post("/api/admin/media/folders", async (request, reply) => {
    if (!requireAdminSession(request, reply)) return;
    try {
      const input = z.object({ name: z.string().trim().min(1).max(120), parentId: z.preprocess(blankToNull, z.string().trim().min(1).max(191).nullable().optional()) }).strict().parse(request.body);
      reply.header("Cache-Control", "no-store");
      return reply.code(201).send({ data: await options.service.createFolder(input.name, input.parentId ?? null) });
    } catch (error) {
      return sendKnownError(error, reply);
    }
  });
  ...
  app.patch("/api/admin/media/assets/:id", async (request, reply) => {
    ...
    try {
      ...
      const asset = await options.service.updateAsset(id, {
        ...(input.displayName === undefined ? {} : { displayName: input.displayName ?? undefined }),
        ...(input.altText === undefined ? {} : { altText: input.altText ?? undefined }),
        ...(input.caption === undefined ? {} : { caption: input.caption ?? undefined }),
