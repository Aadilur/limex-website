...
const optionalHrefSchema = z
  ...
  .max(1000)
  .refine(
    (value) => !value || isSafeServiceHref(value),
    "Use an internal path, hash link, HTTP or HTTPS URL.",
  )
  .default("");
...
const localeQuerySchema = z
  .object({ locale: z.enum(["en", "bn"]).default("en") })
  .strict();
...
const detailSchema = z
  .object({
    ...
    steps: z
      .array(z.object({ title: requiredText(180), description: text(500) }))
      .max(12)
      .default([]),
    facts: z
      .array(z.object({ label: requiredText(120), value: requiredText(180) }))
      .max(12)
      .default([]),
    pricing: z
      .array(
    ...
    faqs: z
      .array(
        z.object({ question: requiredText(300), answer: requiredText(1600) }),
      )
      .max(30)
      .default([]),
...
function sendKnownServiceError(error: unknown, reply: FastifyReply) {
  ...
  if (error instanceof ServiceSafetyError)
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    if (error.code === "P2002")
      return reply
        .code(409)
        .send({ error: "A service with that key or URL slug already exists." });
    if (error.code === "P2025")
      return reply
        .code(404)
        .send({ error: "The requested service was not found." });
  }
  throw error;
}
...
export async function serviceRoutes(
  app: FastifyInstance,
  options: { service: ServiceService },
) {
  ...
  app.get("/api/services/:slug", async (request, reply) => {
    ...
    if (!service)
      return reply
        .code(404)
        .send({ error: "This service page is not published." });
    reply.header(
      "Cache-Control",
      "public, max-age=60, stale-while-revalidate=300",
    );
  ...
  app.post("/api/admin/services", async (request, reply) => {
    ...
    try {
      ...
      return {
        data: await options.service.createService(
          normalizeProfileInput(input.profile),
          session.username,
        ),
      };
    } catch (error) {
  ...
  app.put("/api/admin/services/:id", async (request, reply) => {
    ...
    try {
      ...
      const data = await options.service.saveService(
        id,
        normalizeProfileInput(input.profile),
  ...
  app.post("/api/admin/services/:id/assignment", async (request, reply) => {
    ...
    try {
      ...
      return {
        data: await options.service.assignService(
          id,
          input.menuTarget,
          input.expectedRevision,
          session.username,
  ...
  app.post(
    "/api/admin/services/profile/:id/publish",
    async (request, reply) => {
      const session = adminSession(request, reply);
      if (!session) return;
      try {
        ...
        return {
          data: await options.service.publishService(
            id,
            expectedRevision,
            session.username,
          ),
  ...
  app.post(
    "/api/admin/services/profile/:id/unpublish",
    async (request, reply) => {
      const session = adminSession(request, reply);
      if (!session) return;
      try {
        ...
        const { expectedRevision } = revisionBodySchema.parse(request.body);
        return {
          data: await options.service.unpublishService(
            id,
            expectedRevision,
            session.username,
          ),
