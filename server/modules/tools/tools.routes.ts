...
const valuesSchema = z
  .record(safeText(5000))
  .refine(
    (values) => Object.keys(values).length <= 80,
    "Too many fields were submitted.",
  );
...
const optionalDate = z.preprocess(
  ...
  z
    ...
    }, "Choose a date within the next 180 days.")
    .optional(),
);
const optionalTime = z.preprocess(
  ...
  z
    .string()
    .refine(
      (value) => isContactTimeSlot(value),
      "Choose a 30-minute slot between 09:00 and 18:00.",
    )
    .optional(),
);
  ...
  .superRefine((input, context) => {
    if (!input.phone && !input.email)
      context.addIssue({
        ...
        path: ["phone"],
        message: "Add a phone number or email so we know how to reach you.",
      });
    if (Boolean(input.preferredDate) !== Boolean(input.preferredTime))
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["preferredDate"],
        message: "Choose both a date and time, or leave the schedule blank.",
      });
    if (
      input.requestType === "APPOINTMENT" &&
      (!input.preferredDate || !input.preferredTime)
    )
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["preferredDate"],
        message: "Choose a preferred date and time for an appointment request.",
      });
    if (
      input.toolSlug === "contact" &&
      (!input.services || input.services.length === 0)
    )
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["services"],
        message: "Choose at least one service so we can direct your request.",
      });
    if (input.toolSlug === "contact" && input.values)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["values"],
        message: "Tool values are not accepted on a general enquiry.",
      });
  });

async function configuration() {
  const row = await prisma.businessToolSettings.upsert({
    where: { id: "default" },
    update: {},
    create: {
      id: "default",
      settings: defaultToolsSettings as unknown as Prisma.InputJsonValue,
    },
  });
  return {
    version: row.version,
    settings: normalizeToolsSettings(row.settings),
  };
}
function invalid(reply: FastifyReply, error: unknown) {
  if (error instanceof z.ZodError)
    return reply
      .code(400)
      .send({ error: error.issues[0]?.message ?? "Check your entries." });
  return reply
    .code(400)
    .send({
...
export async function toolsRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        return {
          data: {
            ...calculateTool(slug, input.values, config.settings),
  ...
  app.post(
    "/api/tools/documents/:slug",
    { bodyLimit: 100000 },
    async (request, reply) => {
      if (!documentLimiter(request, reply, "documents")) return;
      const { slug } = paramsSchema.parse(request.params);
      const input = inputSchema.parse(request.body);
      if (getTool(slug)?.group !== "builder")
        return reply.code(400).send({ error: "Choose a document builder." });
      try {
        const values = validateFields(documentFields(slug), input.values);
        if (
          slug === "moa-aoa" &&
          (Number(values.capital) <= 0 ||
            Number(values.capital) % Number(values.shareValue) !== 0)
        )
          throw new Error(
            "Capital must be a positive whole multiple of the share value.",
          );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (input.website)
        return reply
          .code(400)
          .send({ error: "Unable to submit this request." });
      const requestMeta = {
      ...
      try {
        if (input.toolSlug !== "contact") {
          const config = await configuration();
          if (input.values && getTool(input.toolSlug)?.group === "calculator")
            snapshot = {
              ...calculateTool(input.toolSlug, input.values, config.settings),
              configVersion: config.version,
              ...requestMeta,
            };
          else if (input.values) {
            const values = validateFields(
              documentFields(input.toolSlug),
              input.values,
            );
            if (
              input.toolSlug === "moa-aoa" &&
              (Number(values.capital) <= 0 ||
                Number(values.capital) % Number(values.shareValue) !== 0)
            )
              throw new Error("Check authorised capital and share value.");
            snapshot = {
              values,
              draft: createDocumentDraft(input.toolSlug, values),
              templateVersion: 1,
              ...requestMeta,
            };
          } else
      ...
      try {
        const saved = await prisma.toolServiceRequest.create({
          data: {
            ...
            context: snapshot as Prisma.InputJsonValue,
          },
        });
        ...
        if (
          ...
          const existing = await prisma.toolServiceRequest.findUnique({
          ...
          if (
            ...
            existing.toolSlug === input.toolSlug &&
            existing.requestType === input.requestType &&
            existing.preferredDate === (input.preferredDate ?? null) &&
            existing.preferredTime === (input.preferredTime ?? null) &&
            existing.message === input.message
          )
            return { data: { reference: existing.id } };
          return reply
            .code(409)
            .send({
              error:
                "This submission key was already used. Start a new request.",
            });
        }
        throw error;
      }
    ...
    async (request, reply) => {
      ...
      const input = z
        .object({
          ...
          settings: toolsSettingsSchema,
        })
        .strict()
        .parse(request.body);
      const result = await prisma.businessToolSettings.updateMany({
        where: { id: "default", version: input.version },
        data: {
      ...
      if (!result.count)
        return reply
          .code(409)
          .send({
            error: "Settings changed in another session. Reload before saving.",
          });
      return { data: await configuration() };
    },
  );
  app.get("/api/admin/tools/requests", async (request, reply) => {
    if (!requireAdminSession(request, reply)) return;
    reply.header("Cache-Control", "no-store");
    const { page, status, source, search } = z
      .object({
    ...
    const [items, total] = await Promise.all([
      prisma.toolServiceRequest.findMany({
        where,
        take: 20,
        skip: (page - 1) * 20,
        orderBy: { createdAt: "desc" },
      }),
      prisma.toolServiceRequest.count({ where }),
    ]);
  ...
  app.patch("/api/admin/tools/requests/:id", async (request, reply) => {
    ...
    const { status } = z
      ...
      .strict()
      .parse(request.body);
    const updated = await prisma.toolServiceRequest.updateMany({
      where: { id },
      data: { status },
    });
    if (!updated.count)
