...
export function ToolsDirectory() {
  ...
  return <div className={styles.directory}>
    ...
    {filteredTemplates.length ? <section className={styles.group}><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className={styles.groupTitle}>Published templates</h2><p className={styles.muted}>Fill in a reusable document, review it and print it when ready.</p></div><span className="rounded-full bg-[#f2edf7] px-3 py-1 text-[11px] font-semibold text-[#5c4aa6]">{filteredTemplates.length} available</span></div><div className={styles.builderGrid}>{filteredTemplates.map((template) => <Link href={`/business-tools/templates/${template.slug}`} className={styles.builderCard} key={template.id}><ServiceIcon name="contract" className={`${styles.icon} shrink-0`} /><div><strong>{template.title}</strong><p>{template.description}</p></div><span className={styles.arrow} aria-hidden="true">↗</span></Link>)}</div></section> : null}
