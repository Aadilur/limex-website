...
export function TemplateWorkspace({ slug, preview = false }: { slug: string; preview?: boolean }) {
  ...
  return <div className="min-w-0 space-y-6">
    ...
    <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,0.84fr)_minmax(420px,1.16fr)]">
      ...
      <section className="min-w-0 overflow-hidden rounded-[22px] border border-[#e1dcd4] bg-[#f7f4ef] p-3 sm:p-5" aria-label="Document preview"><div className="mb-3 flex flex-wrap items-center justify-between gap-2 px-1"><div><p className="text-[11px] font-bold uppercase tracking-[0.13em] text-[#e44762]">Live preview</p><p className="mt-1 text-[12px] text-[#817970]">{template.settings.paperSize} ratio · {template.pages.length} {template.pages.length === 1 ? "page" : "pages"}</p></div><span className="text-[12px] font-semibold text-[#817970]">Empty fields show labels</span></div><div className="min-w-0 overflow-x-auto rounded-[15px] bg-[#eee9e2] p-2 sm:p-3"><div className="mx-auto w-full max-w-[820px] min-w-0"><DocumentTemplatePaper template={template} values={values} showLabels compact /></div></div></section>
    </div>
  </div>;
}
