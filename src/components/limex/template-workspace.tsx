...
import { isTemplateFieldVisible, missingTemplateFields, templatePaperSizeLabels, type PublicDocumentTemplate, type TemplateField, type TemplateValues } from "@/lib/document-templates";
...
export function TemplateWorkspace({ slug, preview = false }: { slug: string; preview?: boolean }) {
  ...
  const visibleFields = useMemo(() => template ? template.fields.filter((field) => isTemplateFieldVisible(field, values)) : [], [template, values]);
