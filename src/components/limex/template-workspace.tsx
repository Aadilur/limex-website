...
import { getPublishedTemplate } from "@/lib/template-api";
...
export function TemplateWorkspace({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    let active = true;
    void getPublishedTemplate(slug).then((loaded) => {
      if (!active) return;
      setTemplate(loaded);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,0.84fr)_minmax(420px,1.16fr)]">
      ...
      <section className="min-w-0 rounded-[22px] border border-[#e1dcd4] bg-[#f7f4ef] p-3 sm:p-5" aria-label="Document preview"><div className="mb-3 flex items-center justify-between gap-3 px-1"><div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[#e44762]">Live preview</p><p className="mt-1 text-[12px] text-[#817970]">Page setup: {template.settings.paperSize} · {template.blocks.filter((block) => block.type === "pageBreak").length + 1} pages</p></div><span className="text-[11px] font-semibold text-[#817970]">Updates as you type</span></div><DocumentTemplatePaper template={template} values={values} showLabels compact /></section>
    </div>
  </div>;
}
