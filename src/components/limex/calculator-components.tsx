...
function StepBadge({ number }: { number: string }) {
  return <span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#0055ff] text-nav-compact font-bold text-white">{number}</span>;
}
...
function CalculatorHeading({
  ...
  return (
    <div className="flex flex-col gap-cluster-sm">
      ...
      <h2 className="max-w-[620px] font-brand text-section-title text-[#071b3d]">{title}</h2>
  ...
  return (
    <div className="flex items-start gap-cluster-sm">
      ...
      <div className="min-w-0">
        <h3 className="text-body-xs font-semibold text-[#071b3d]">{title}</h3>
...
function MoneyInput({
  ...
  return (
    <label className="block min-w-0" htmlFor={id}>
      <span className={`${compact ? "text-micro" : "text-body-xs"} font-semibold text-[#071b3d]`.trim()}>{label}</span>
      ...
      <span className={`mt-cluster-sm flex items-center gap-cluster-sm rounded-control border border-[#e5e0d6] bg-white px-3.5 transition-colors focus-within:border-pink focus-within:ring-2 focus-within:ring-pink/10 ${compact ? "h-control-sm" : "h-control"}`.trim()}>
        ...
        <input
          className={`${compact ? "text-micro" : "text-body-sm"} min-w-0 flex-1 bg-transparent font-semibold text-[#071b3d] outline-none placeholder:text-[#a7aab4]`.trim()}
          id={id}
          inputMode="decimal"
          min="0"
  ...
  return (
    <label className="flex min-w-0 flex-col gap-cluster-sm rounded-control border border-[#e5e0d6] bg-white px-2.5 py-2.5" htmlFor={id}>
      ...
      <span className="flex items-center gap-cluster-sm">
        ...
        <input
          className="min-w-0 flex-1 bg-transparent text-right text-micro font-semibold text-[#071b3d] outline-none placeholder:text-[#a7aab4]"
          id={id}
          inputMode="decimal"
          min="0"
          onChange={(event) => onChange(event.target.value)}
...
function ChoiceButton({
  ...
  return (
    <button
      ...
      className={`inline-flex min-h-9 items-center justify-center rounded-control border px-3 text-center text-button font-semibold transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-2 ${selected ? "border-[#071b3d] bg-[#071b3d] text-white" : "border-[#e5e0d6] bg-white text-[#071b3d] hover:border-[#0055ff]"} ${className}`.trim()}
      onClick={onClick}
      type="button"
    >
      {children}
    ...
    <section className="scroll-mt-[120px] overflow-hidden rounded-panel border border-[#e3ded4] bg-[#fbfaf6]" id="limited-company-calculator" aria-labelledby="limited-company-title">
      <div className="grid gap-section-gap bg-[#f9f8f5] px-page-gutter py-section-y sm:px-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.75fr)] lg:items-stretch lg:px-12 lg:py-9">
        <div className="flex flex-col justify-center gap-cluster-sm">
          <p className="text-overline text-[#0055ff]">BUSINESSBOX / LIMITED COMPANY</p>
        ...
        <aside className="flex min-h-[174px] flex-col justify-between gap-section-gap rounded-card bg-[#071b3d] px-card-pad py-card-pad text-white sm:px-[22px]">
          <div>
            <p className="text-overline text-[#d5f8ff]">CALCULATOR SCOPE</p>
            <h3 className="mt-1 font-brand text-subheading">One transparent estimate</h3>
            <p className="mt-cluster-sm max-w-[390px] text-micro text-[#d5f8ff]">RJSC fee, documentation, Trade License, and optional services in one view.</p>
          </div>
          <div className="grid grid-cols-2 gap-cluster-sm">
            <div className="rounded-control bg-[#0039b8] px-2.5 py-1.5">
              <strong className="block text-body-sm">15</strong>
              <span className="block text-nav-compact text-[#d5f8ff]">authorized capital options</span>
            </div>
            <div className="rounded-control bg-[#0039b8] px-2.5 py-1.5">
              <strong className="block text-body-sm">2</strong>
              <span className="block text-nav-compact text-[#d5f8ff]">trademark and BIN add-ons</span>
            </div>
          </div>
        </aside>
      </div>
      ...
      <div className="border-t border-[#e3ded4] bg-[#fbfaf6] px-page-gutter py-section-y sm:px-8 lg:px-12 lg:py-section-y">
        <div className="flex flex-col gap-cluster-xs sm:flex-row sm:items-end sm:justify-between sm:gap-cluster">
          ...
          <span className="text-overline text-[#0055ff]">LIVE CALCULATION</span>
        </div>
          ...
          <div className="flex flex-col gap-cluster-sm rounded-nav border border-[#e5e0d6] bg-white p-card-pad-sm sm:p-card-pad">
            <div className="rounded-control border border-[#f0d7da] bg-white p-cluster">
              ...
              <div className="mt-cluster-sm flex flex-col gap-cluster-sm sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="flex items-center justify-between gap-cluster-sm text-micro text-[#6e6b66] sm:min-w-[170px] sm:justify-end">
                  RJSC fee <strong className="text-meta font-semibold text-[#0055ff]">{formatTaka(selectedCapital.fee)}</strong>
                </span>
              </div>
            </div>
            ...
            <div className="rounded-control border border-[#e5e0d6] bg-white p-cluster">
              ...
              <div className="mt-cluster-sm flex flex-col gap-cluster-sm sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="flex items-center justify-between gap-cluster-sm text-micro text-[#6e6b66] sm:min-w-[170px] sm:justify-end">
                  Trade License <strong className="text-meta font-semibold text-[#0055ff]">{formatTaka(tradeLicenseFee)}</strong>
                </span>
              </div>
            </div>
            ...
            <div className="rounded-control border border-[#e5e0d6] bg-white p-cluster">
              ...
              <div className="mt-cluster-sm grid gap-cluster-sm sm:grid-cols-2">
                <button aria-pressed={location === "dhaka"} className={`flex min-h-14 items-center gap-cluster-sm rounded-control border px-3 text-left transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-2 ${location === "dhaka" ? "border-[#ebc4c9] bg-[#fbeced]" : "border-[#e3ded4] bg-white hover:border-[#0055ff]"}`.trim()} onClick={() => setLocation("dhaka")} type="button">
                  <span className={`grid size-4 shrink-0 place-items-center rounded-[8px] border ${location === "dhaka" ? "border-[#0055ff] bg-[#0055ff]" : "border-[#ebc4c9] bg-white"}`.trim()}>{location === "dhaka" ? <span className="size-1.5 rounded-[3px] bg-white" /> : null}</span>
                  <span><strong className={`block text-micro font-semibold ${location === "dhaka" ? "text-[#0055ff]" : "text-[#1a1a1a]"}`.trim()}>Inside Dhaka City Corporation</strong><span className="mt-0.5 block text-nav-compact text-[#6e6b66]">Trade License fee: {formatTaka(selectedPaidUp.fee)}</span></span>
                </button>
                <button aria-pressed={location === "outside"} className={`flex min-h-14 items-center gap-cluster-sm rounded-control border px-3 text-left transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-2 ${location === "outside" ? "border-[#ebc4c9] bg-[#fbeced]" : "border-[#e3ded4] bg-white hover:border-[#0055ff]"}`.trim()} onClick={() => setLocation("outside")} type="button">
                  <span className={`grid size-4 shrink-0 place-items-center rounded-[8px] border ${location === "outside" ? "border-[#0055ff] bg-[#0055ff]" : "border-[#ebc4c9] bg-white"}`.trim()}>{location === "outside" ? <span className="size-1.5 rounded-[3px] bg-white" /> : null}</span>
                  <span><strong className={`block text-micro font-semibold ${location === "outside" ? "text-[#0055ff]" : "text-[#1a1a1a]"}`.trim()}>Outside Dhaka City Corporation</strong><span className="mt-0.5 block text-nav-compact text-[#6e6b66]">Estimated fee: {formatTaka(tradeLicenseFee)}</span></span>
                </button>
              </div>
            </div>
            ...
            <div className="rounded-control border border-[#e5e0d6] bg-white p-cluster">
              ...
              <div className="mt-cluster-sm grid gap-cluster-sm sm:grid-cols-2">
                <label className={`flex min-h-14 cursor-pointer items-center gap-cluster-sm rounded-control border px-3 transition-colors ${trademark ? "border-[#ebc4c9] bg-[#fbeced]" : "border-[#e3ded4] bg-white hover:border-[#0055ff]"}`.trim()} htmlFor="trademark-addon">
                  ...
                  <span className={`grid size-4 shrink-0 place-items-center rounded-[4px] border text-nav-compact font-bold text-white transition-colors ${trademark ? "border-[#0055ff] bg-[#0055ff]" : "border-[#e3ded4] bg-white"}`.trim()}>{trademark ? "✓" : null}</span>
                ...
                <label className={`flex min-h-14 cursor-pointer items-center gap-cluster-sm rounded-control border px-3 transition-colors ${bin ? "border-[#ebc4c9] bg-[#fbeced]" : "border-[#e3ded4] bg-white hover:border-[#0055ff]"}`.trim()} htmlFor="bin-addon">
                  <input checked={bin} className="peer sr-only" id="bin-addon" onChange={(event) => setBin(event.target.checked)} type="checkbox" />
                  <span className={`grid size-4 shrink-0 place-items-center rounded-[4px] border text-nav-compact font-bold text-white transition-colors ${bin ? "border-[#0055ff] bg-[#0055ff]" : "border-[#e3ded4] bg-white"}`.trim()}>{bin ? "✓" : null}</span>
  ...
  return (
    <section className="scroll-mt-[120px] rounded-panel border border-[#e5e0d6] bg-[#fcfbf7] p-card-pad-sm sm:p-section-y lg:p-9" id="tax-calculator" aria-labelledby="tax-calculator-title">
      ...
      <div className="mt-section-y flex items-end justify-between gap-cluster border-b border-[#e5e0d6] pb-3">
        <div>
          <h3 className="font-brand text-subheading text-[#071b3d]">Calculation details</h3>
      ...
      <div className="mt-section-gap-lg grid gap-section-gap lg:grid-cols-[minmax(0,1fr)_minmax(220px,0.56fr)] lg:gap-cluster-lg">
        <div className="flex min-w-0 flex-col gap-section-gap">
          <div>
            <div className="flex flex-wrap items-center justify-between gap-cluster-sm">
              <h3 className="text-body-xs font-semibold text-[#071b3d]">1. Taxpayer category</h3>
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-cluster-sm">
              <h3 className="text-body-xs font-semibold text-[#071b3d]">2. Annual income</h3>
            ...
            <div className="mt-cluster-sm grid gap-cluster-sm sm:grid-cols-2">
              <div className="flex min-h-11 items-center justify-between gap-cluster-sm rounded-control bg-[#f7f7f9] px-3"><span className="text-meta text-[#6b7487]">Exempted income</span><strong className="text-button text-[#071b3d]">{formatTaka(calculation.exempted)}</strong></div>
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-cluster-sm">
              <h3 className="text-body-xs font-semibold text-[#071b3d]">3. Investment &amp; source tax</h3>
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-cluster-sm">
              <h3 className="text-button font-semibold text-[#071b3d]">Detailed investment</h3>
        ...
        <aside className="min-w-0 rounded-card bg-[#fcfbf7] p-card-pad-sm">
          <div>
            <h3 className="text-body font-semibold text-[#071b3d]">Live summary</h3>
            <p className="mt-0.5 text-micro text-[#6b7487]">Updates as you type</p>
          </div>
          <div className="mt-cluster rounded-control bg-[#071b3d] p-card-pad-sm text-white">
          ...
          <h4 className="mt-cluster text-button font-semibold text-[#071b3d]">Calculation breakdown</h4>
          <div className="mt-cluster-sm rounded-control border border-[#e5e0d6] bg-white p-3">
            ...
            <div className="flex items-center justify-between gap-cluster-sm pt-2 text-micro font-semibold text-[#071b3d]"><span>Payable with return</span><strong>{formatTaka(calculation.payable)}</strong></div>
          </div>
          <div className="mt-cluster-sm flex gap-cluster-sm rounded-control border border-[#e5e0d6] bg-white p-1">
            <span className="flex min-h-7 flex-1 items-center justify-center rounded-control bg-[#071b3d] text-micro font-semibold text-white">Summary</span>
            <a className="flex min-h-7 flex-1 items-center justify-center rounded-control text-micro font-medium text-[#6b7487] transition-colors hover:bg-[#f7f7f9] hover:text-[#071b3d]" href="#tax-rules">Rules</a>
          </div>
        </aside>
      </div>
  ...
  return (
    <section className="scroll-mt-[120px] rounded-panel border border-[#e5e0d6] bg-[#fcfbf7] p-card-pad-sm sm:p-section-y lg:p-section-y" id="vat-calculator" aria-labelledby="vat-calculator-title">
      ...
      <div className="mt-section-gap-lg rounded-card border border-[#e5e0d6] bg-white p-card-pad-sm sm:p-card-pad">
        <h3 className="font-brand text-subheading text-[#071b3d]" id="vat-calculator-title">Invoice estimate</h3>
        ...
        <div className="mt-section-gap-lg grid grid-cols-2 gap-1 rounded-control border border-[#e5e0d6] bg-[#fcfbf7] p-1">
          <button aria-pressed={basis === "exclusive"} className={`min-h-9 rounded-control text-button font-semibold transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-2 ${basis === "exclusive" ? "bg-pink text-white" : "text-[#071b3d] hover:bg-white"}`.trim()} onClick={() => setBasis("exclusive")} type="button">VAT exclusive</button>
          <button aria-pressed={basis === "inclusive"} className={`min-h-9 rounded-control text-button font-semibold transition-colors focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-2 ${basis === "inclusive" ? "bg-pink text-white" : "text-[#071b3d] hover:bg-white"}`.trim()} onClick={() => setBasis("inclusive")} type="button">VAT inclusive</button>
        </div>
        ...
        <div className="mt-cluster-sm grid gap-cluster sm:grid-cols-2">
          <label className="flex min-h-[52px] items-center gap-cluster-sm rounded-control border border-[#e5e0d6] bg-white px-3 py-2" htmlFor="vat-tax-year">
            <span className="min-w-0 flex-1"><span className="block text-overline text-[#6b7487]">TAX YEAR</span><select className="mt-0.5 w-full bg-transparent text-body-xs font-semibold text-[#071b3d] outline-none" id="vat-tax-year" onChange={(event) => setTaxYear(event.target.value)} value={taxYear}><option value="current">Current (2026)</option><option value="2025-26">2025–26</option></select></span>
        ...
        <div className="mt-section-gap-lg">
          <div className="flex flex-wrap items-center justify-between gap-cluster-sm"><h4 className="text-body-xs font-semibold text-[#071b3d]">VAT rate</h4><span className="text-meta text-[#6b7487]">Select applicable rate</span></div>
        ...
        <div className="mt-section-gap-lg rounded-control bg-[#071b3d] px-4 py-3.5 text-white">
        ...
        <div className="mt-cluster flex justify-end"><button className="text-meta font-semibold text-pink transition-colors hover:text-[#0055ff]" onClick={reset} type="button">Reset calculator ↺</button></div>
      </div>
    </section>
  );
}
