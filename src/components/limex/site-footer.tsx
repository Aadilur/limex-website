...
export function SiteFooter({ content }: { content?: FooterContent }) {
  const [footer, setFooter] = useState<FooterContent>(
    content ?? defaultLandingContent.footer,
  );
  ...
  return (
    <footer
      className="min-h-0 rounded-panel-mobile bg-navy px-page-gutter pb-section-y-lg pt-section-y-lg text-white lg:min-h-[440px] lg:rounded-panel lg:px-page-gutter-lg lg:pb-6"
      aria-label="Footer"
    >
      <div className="flex flex-col items-start justify-between gap-section-gap lg:flex-row">
        <div>
          ...
          <p className="mt-cluster-sm text-footer text-soft-muted">
            {footer.tagline}
          </p>
        </div>
        <ActionButton
          ...
          variant="white"
          arrow="none"
          className="mt-cluster-lg min-h-button-lg w-full max-w-[250px] justify-center text-body-sm lg:mt-px lg:w-[170px]"
        >
          {footer.ctaLabel}
        </ActionButton>
      </div>
      <h2 className="mt-section-gap-xl max-w-[660px] font-brand text-section-title-mobile lg:mt-section-gap-lg lg:text-section-title">
        {footer.title}
      </h2>
        ...
        {footer.columns
          ...
          .map((column) => (
            <div className="flex flex-col gap-cluster-sm" key={column.id}>
              <h3 className="mb-cluster-sm text-micro font-bold uppercase tracking-eyebrow text-soft-muted">
                {column.title}
              </h3>
              {column.links
                ...
                .map((link) => {
                  ...
                  return isInternal ? (
                    <Link
                      className="text-footer text-white transition-colors hover:text-brand-cyan"
                      href={link.href}
                      key={link.id}
                    >
                      {link.label}
