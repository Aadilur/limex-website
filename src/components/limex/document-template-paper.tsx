...
function formattedStyle(block: Extract<TemplateBlock, { type: "title" | "heading" | "paragraph" | "field" }>, defaultFontSize: ReturnType<typeof resolvePageSettings>["defaultFontSize"], fontScale: number) {
...
function RenderBlock({ block, template, values, showLabels, defaultFontSize }: { block: TemplateBlock; template: DocumentTemplateDraft; values: TemplateValues; showLabels: boolean; defaultFontSize: ReturnType<typeof resolvePageSettings>["defaultFontSize"] }) {
  ...
  if (block.type === "title" || block.type === "heading" || block.type === "paragraph") {
    ...
    return <p className={`mb-3 whitespace-pre-wrap break-words ${formattedClass(block)}`} style={formattedStyle(block, defaultFontSize, template.settings.fontScale)}>{text}</p>;
  }
