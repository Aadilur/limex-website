...
const fieldLabelTextClassName = "text-body-xs font-semibold text-[#49313a]";
  ...
  return (
    <section className="flex min-h-0 flex-col gap-cluster-sm bg-page px-page-gutter pt-section-gap-xl pb-page-gutter lg:grid lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-cluster-lg lg:rounded-panel lg:px-page-gutter-lg lg:pt-section-gap-xl lg:pb-section-y-lg" id="contact" aria-labelledby="contact-title">
      <div className="flex min-h-0 flex-col lg:min-h-[480px] lg:py-2">
        ...
        <div className="relative mt-section-gap-xl max-w-[360px] border-l border-[#ead2d7] pl-4">
          <p className="text-overline text-accent">START HERE</p>
          <p className="mt-cluster-sm max-w-[270px] text-body-xs text-muted">From first question to final filing.</p>
        </div>

        <div className="relative mt-auto border-t border-[#ead2d7] pt-section-gap-lg">
          <p className="text-body-xs font-text text-muted">Prefer a direct line?</p>
          <ActionButton href="#contact-form" variant="dark" className="mt-cluster w-max min-h-11 border-accent bg-accent hover:border-[#c53f62] hover:bg-[#c53f62]">Chat on WhatsApp</ActionButton>
      ...
      <form className="relative flex min-h-0 flex-col rounded-[24px] border border-[#c8c6be] bg-page p-card-pad-sm shadow-[0_12px_30px_rgba(42,44,39,0.04)] lg:min-h-[480px] lg:p-[32px]" id="contact-form" aria-labelledby="contact-form-title" onSubmit={handleSubmit}>
        ...
        <div className="border-b border-[#d3d0c8] pb-section-gap-lg">
          <p className="text-overline text-accent">YOUR DETAILS</p>
