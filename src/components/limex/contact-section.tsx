...
function findServiceOption(
  ...
  // 1. Exact match by value, label, or suffix
  const exact = allOptions.find((option) => {
  ...
  // 2. Slug match (replace hyphens with spaces)
  const dehyphenated = normalized.replace(/-/g, " ");
  const slugMatch = allOptions.find((option) => {
    const label = option.label.toLowerCase();
    const value = option.value.toLowerCase();
    return label === dehyphenated || value.endsWith(`: ${dehyphenated}`);
  });
  if (slugMatch) return slugMatch;

  // 3. Substring match
  const partial = allOptions.find((option) => {
    const label = option.label.toLowerCase();
    return (
      label.includes(normalized) ||
      normalized.includes(label) ||
      label.includes(dehyphenated) ||
      dehyphenated.includes(label)
    );
  });
  if (partial) return partial;

  // 4. Fallback option so custom or localized service is always selectable
  return { label: requestedService.trim(), value: requestedService.trim() };
}
...
function ServiceMultiSelect({
  ...
  const options = useMemo(() => {
    const base = groups.flatMap((group) => group.items);
    const missing = value
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        <span
          className={`min-w-0 truncate ${selectedOptions.length ? "text-[#172019]" : "text-[#777872]"}`.trim()}
        >
          {summary}
        </span>
        <ChevronDownIcon
      ...
      {isOpen ? (
        <div
          className="absolute inset-x-0 top-[calc(100%+8px)] z-30 overflow-hidden rounded-[16px] border border-[#d0cdc4] bg-[#f8f6f1] shadow-[0_18px_42px_rgba(42,44,39,0.12)]"
          id={optionsId}
          role="listbox"
        ...
        >
          <div className="flex items-center justify-between border-b border-[#dedbd3] px-3.5 py-2.5">
            <span className="text-micro font-semibold text-[#4c4d48]">
              Choose services
            </span>
            <button
              className="text-micro font-semibold text-accent transition-colors hover:text-[#bd3f60] disabled:cursor-not-allowed disabled:text-[#aaa9a2]"
              type="button"
              onClick={() => onChange([])}
          ...
          <div className="max-h-64 overflow-y-auto p-2">
            {groups.map((group) => (
              <div className="not-last:pb-2" key={group.label}>
                <p className="px-2.5 pb-1 pt-1 text-overline text-[#6c6c65]">
                  {group.label}
                </p>
                <div className="grid gap-0.5">
                  {group.items.map((option) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`mt-0.5 grid size-4 shrink-0 place-items-center rounded-[5px] border ${selected ? "border-accent bg-accent text-white" : "border-[#c5c3bb] bg-page text-transparent"}`.trim()}
                          aria-hidden="true"
                        >
...
export function ContactForm({
  ...
  const [values, setValues] = useState<ContactValues>(() => {
    const trimmed = initialService?.trim();
    return {
      ...initialValues,
      services: trimmed ? [trimmed] : [],
  ...
  const [scheduleDateBounds, setScheduleDateBounds] = useState({
    minimum: "",
    maximum: "",
  });
  const [blogSource, setBlogSource] = useState<ContactRequestSource | null>(
    source,
  );
  const submission = useRef<{ key: string; fingerprint: string } | null>(null);
  const serviceGroups = useMemo(
    () => getServiceGroups(menuNavigation),
    [menuNavigation],
  );
  ...
  const updateScheduleDate = (preferredDate: string) => {
    ...
    setValues((current) => ({
      ...current,
      preferredDate,
      preferredTime: preferredDate ? current.preferredTime : "",
    }));
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (values.services.length === 0) {
      setSubmitted(false);
      setFormError(
        "Choose at least one service so we can direct your request.",
      );
      return;
    }
    ...
    const requestType =
      values.preferredDate && values.preferredTime ? "APPOINTMENT" : "CALLBACK";
    const payload = {
      toolSlug: "contact",
      requestType,
    ...
    setBusy(true);
    try {
      const saved = await request<{ reference: string }>(
        "/api/tools/requests",
        {
          method: "POST",
          body: JSON.stringify({
            ...payload,
            submissionId: submission.current.key,
          }),
        },
      );
  ...
  useEffect(() => {
    let cancelled = false;
    setScheduleDateBounds({
      minimum: getDhakaDateValue(),
      maximum: getDhakaDateValueAfter(180),
    });

    const params = new URLSearchParams(window.location.search);
    const article = params.get("article");
    const requestedService =
      initialService ?? source?.service ?? params.get("service") ?? "";
    const urlService = params.get("service") || undefined;
    const urlSource: ContactRequestSource | null =
      params.get("from") === "blog" && article
        ? {
            type: "BLOG",
            slug: article,
    ...
    const setInitialService = (groups: ServiceGroup[]) => {
      ...
      if (!match) return;
      setValues((current) => {
        const hasCustomUserSelection =
          current.services.length > 0 &&
          !current.services.includes(requestedService.trim()) &&
          !current.services.includes(initialService?.trim() ?? "") &&
          !current.services.includes(match.value);

        if (hasCustomUserSelection) return current;
        return { ...current, services: [match.value] };
      });
    };
  ...
  return (
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 gap-cluster lg:grid-cols-2">
          <div className={fieldLabelClassName}>
            ...
            <ServiceMultiSelect
              ...
              onChange={(nextValue) => updateValue("services", nextValue)}
        ...
        {formError ? (
          <p
            className="-mt-1 text-body-xs font-semibold text-accent"
            role="alert"
          >
            {formError}
          </p>
          ...
          <div className="grid grid-cols-1 gap-cluster sm:grid-cols-2">
            ...
            <label className={fieldLabelClassName}>
              ...
              <select
                ...
                disabled={busy || !values.preferredDate}
              >
                <option value="">
      ...
      <div className="mt-auto pt-section-gap-lg">
        ...
        >
          ...
          <span
            className="text-icon-action transition-transform duration-200 group-hover:translate-x-0.5"
            aria-hidden="true"
          >
            {busy ? "…" : submitted ? "✓" : "↗"}
          </span>
...
export function ContactSection({
  ...
  return (
    <section
      className="flex min-h-0 flex-col gap-cluster-sm bg-page px-page-gutter pt-section-gap-xl pb-page-gutter lg:grid lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-cluster-lg lg:rounded-panel lg:px-page-gutter-lg lg:pt-section-gap-xl lg:pb-section-y-lg"
      id="contact"
      ...
      <div className="flex min-h-0 flex-col items-center text-center lg:min-h-[560px] lg:justify-center lg:py-6">
        <div className="flex w-full max-w-[430px] flex-col items-center">
          <div className="max-w-[400px]">
            <h2
              className="mt-0 max-w-[390px] text-balance font-brand text-section-title-mobile text-ink lg:text-section-title"
              id="contact-title"
            >
              {content.title}
            </h2>
            <p className="mt-cluster-lg max-w-[360px] text-body-sm text-muted">
              {content.description}
            </p>
          </div>
        </div>
        ...
        <div className="relative mt-section-gap-lg w-full max-w-[360px] border-t border-[#ead2d7] pt-section-gap-lg lg:mt-section-gap-xl">
          <p className="text-body-xs font-text text-muted">
            {content.directLineLabel}
          </p>
          <ActionButton
            href={contactDetails?.whatsappUrl || "#contact-form"}
            variant="dark"
            className="mt-cluster w-max min-h-11 border-accent bg-accent hover:border-[#c53f62] hover:bg-[#c53f62]"
          >
            {content.directCtaLabel}
          </ActionButton>
          <div className="mt-section-gap-lg grid w-full grid-cols-1 gap-cluster-lg text-center sm:grid-cols-2">
            <div className="flex flex-col gap-cluster-sm">
              ...
              <strong className="text-body-sm font-semibold text-ink">
                {whatsappDisplay}
              </strong>
            </div>
            <div className="flex flex-col gap-cluster-sm">
              <span className="text-overline text-accent">EMAIL</span>
              <strong className="break-words text-body-sm font-semibold text-ink sm:whitespace-nowrap">
                {email}
              </strong>
            </div>
          </div>
...
export function ContactModal({
  ...
  const whatsappDisplay =
    contactDetails?.whatsappDisplay?.trim() || content.whatsapp;
