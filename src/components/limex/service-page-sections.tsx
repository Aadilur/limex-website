  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-cluster">
        ...
        <a
          className="shrink-0 text-button font-semibold text-pink transition-colors hover:text-ink focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-2"
          href={
            locale === "bn"
      ...
      <div className="mt-section-gap-xl grid gap-section-gap lg:grid-cols-[minmax(0,1fr)_minmax(360px,460px)] lg:items-center lg:gap-section-gap-xl">
        <div className="min-w-0">
          ...
          <h1
            className="mt-cluster max-w-[690px] font-brand text-page-title text-ink max-lg:text-page-title-mobile"
            id="service-page-title"
          >
            {service.title}
          </h1>
          <p className="mt-cluster max-w-[620px] text-body-lg text-muted">
            {service.description}
          </p>
          <div className="mt-section-gap-lg flex flex-wrap items-center gap-cluster">
            <ActionButton
              ...
              arrow="cta"
              className="min-h-button-lg w-[188px] pl-5 text-body-xs"
            >
              {service.ctaLabel}
            </ActionButton>
            {service.destination &&
            ...
            service.destination.type !== "CONTACT" ? (
              <a
                className="inline-flex min-h-control items-center gap-cluster-sm rounded-pill px-3 text-button font-semibold text-ink transition-colors hover:text-pink focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-3"
          ...
          <dl className="mt-section-gap-xl grid max-w-[650px] grid-cols-1 gap-section-gap border-t border-[#e0dee3] pt-card-pad-sm sm:grid-cols-3 sm:gap-cluster">
            <div>
              <dt className="text-overline text-[#0055ff]">
                {ui.startingPrice}
              </dt>
              <dd className="mt-cluster-xs text-body-xs font-semibold text-ink">
        ...
        <div
          className="relative min-h-[320px] overflow-hidden rounded-panel bg-[#f4eff9] sm:min-h-[360px]"
        ...
        >
          ...
          />
          {!service.mediaUrl?.trim() ? (
            <p className="absolute left-6 top-5 text-overline text-[#0055ff]">
              {ui.mediaSlot}
            </p>
          ) : null}
          <div
            className={`absolute left-1/2 top-1/2 w-[min(336px,calc(100%-48px))] -translate-x-1/2 -translate-y-1/2 rounded-card px-card-pad py-section-y ${service.mediaUrl?.trim() ? "bg-navy/85 text-white" : "bg-white"}`.trim()}
          >
            <h2
              className={`font-brand text-section-title ${service.mediaUrl?.trim() ? "text-white" : "text-ink"}`.trim()}
            >
              {service.mediaTitle}
            </h2>
            <p
              className={`mt-cluster text-body-xs ${service.mediaUrl?.trim() ? "text-white/75" : "text-muted"}`.trim()}
            >
              {service.mediaDescription}
            </p>
          </div>
          {!service.mediaUrl?.trim() ? (
            <p className="absolute bottom-5 right-6 text-micro font-medium text-muted">
              {ui.optionalMedia}
            </p>
          ) : null}
        </div>
...
export function ServiceOverviewSection({
  ...
  return (
    <section
      className="mt-section-gap-xl border-t border-[#e0dee3] bg-page pt-section-y lg:mt-section-gap-xl lg:pt-section-y-xl"
      id="service-overview"
      aria-labelledby="service-overview-title"
    >
      <div
        className={`grid gap-section-gap ${service.facts.length ? "lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.7fr)] lg:gap-section-gap-xl" : ""}`.trim()}
      >
        <div className="min-w-0">
          {overviewHtml ? (
            <RichTextContent
            ...
            <>
              <p className="text-overline text-[#0055ff]">
                {service.overviewEyebrow}
              </p>
              <h2
                className="mt-cluster max-w-[760px] font-brand text-page-title text-ink max-lg:text-page-title-mobile"
                id="service-overview-title"
              >
              ...
              <RichTextContent
                ...
                className={`${richTextClass} mt-cluster max-w-[720px]`}
              />
              ...
              <article className="mt-section-gap-lg rounded-nav border border-[#e0dee3] bg-white px-card-pad py-card-pad lg:px-section-y lg:py-section-y">
                ...
                <h3 className="mt-cluster font-brand text-section-title text-ink">
              ...
              {service.benefits?.length ? (
                <div className="mt-section-gap-lg grid gap-cluster sm:grid-cols-2">
                  {service.benefits.map((benefit) => (
                    <p
                      className="flex items-start gap-cluster-sm text-body-sm text-ink"
                      key={benefit}
                    >
                      <span
                        className="mt-2 size-2 shrink-0 rounded-full bg-[#0055ff]"
                        aria-hidden="true"
                      />
                      {benefit}
                    </p>
              ...
              {service.steps?.length ? (
                <div className="mt-section-gap-xl grid gap-cluster sm:grid-cols-3">
                  {service.steps.map((step, index) => (
                    <div
                      className="border-t border-[#e0dee3] pt-cluster"
                      key={`${step.title}-${index}`}
                    >
                      <span className="text-overline text-[#0055ff]">
                        {String(index + 1).padStart(2, "0")}
                      </span>
                      <h3 className="mt-cluster-xs font-brand text-subheading-mobile text-ink">
                        {step.title}
                      </h3>
          ...
          <aside className="lg:pt-[166px]" aria-label="Service key facts">
            ...
            <dl className="mt-cluster divide-y divide-[#e0dee3] border-y border-[#e0dee3]">
              {service.facts.map((fact) => (
                <div
                  className="grid grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] gap-cluster py-4"
                  key={fact.label}
                >
                  <dt className="text-meta text-muted">{fact.label}</dt>
                  <dd className="text-body-xs font-semibold text-ink">
                    {fact.value}
                  </dd>
                </div>
              ))}
        ...
        <div className="mt-section-gap-xl border-t border-[#e0dee3] pt-section-y">
          ...
          <div className="mt-cluster flex flex-wrap gap-2">
            {service.relatedLinks.map((link) => (
              <a
                className="inline-flex min-h-10 items-center gap-2 rounded-pill bg-white px-3.5 text-button font-semibold text-ink ring-1 ring-[#e0dee3] transition-colors hover:text-pink hover:ring-[#0055ff]/40"
                href={link.href}
      ...
      {helpfulTools.length ? (
        <div className="mt-section-gap-xl border-t border-[#e0dee3] pt-section-y">
          ...
          <p className="mt-cluster max-w-[860px] text-body-sm text-muted">
            {toolsDescription}
          </p>
            ...
            >
              ...
              <div className="mt-cluster grid gap-cluster sm:grid-cols-2 lg:grid-cols-3">
                {builderTools.map((tool) => (
                  <a
                    className="group flex min-w-0 items-center justify-between gap-cluster rounded-card bg-white px-card-pad-sm py-cluster ring-1 ring-[#e0dee3] transition-colors hover:ring-[#0055ff]/50 focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-3"
                    href={toolHref(tool.slug)}
                    key={tool.slug}
                  >
                    <span className="min-w-0">
                      <span className="block truncate text-body-xs font-semibold text-ink group-hover:text-pink">
                        {tool.title}
                      </span>
                      <span className="mt-cluster-xs block truncate text-micro text-muted">
                        {tool.description}
                      </span>
                    </span>
...
function PriceCard({
  ...
  return (
    <article
      className={`relative flex min-h-[292px] flex-col rounded-nav border bg-white p-card-pad ${tier.featured ? "border-[#0055ff]" : "border-[#e0dee3]"}`.trim()}
    >
      {tier.featured ? (
        <WaveLabel className="absolute left-5 top-4 text-[#0055ff]">
          {mostPopularLabel}
        </WaveLabel>
      ) : null}
      <p
        className={`text-body font-semibold ${tier.featured ? "mt-8 text-[#0055ff]" : "text-muted"}`.trim()}
      >
        {tier.name}
      </p>
      <p className="mt-cluster-sm font-brand text-section-title text-ink">
        {tier.price}
      </p>
      <p className="mt-cluster-sm text-body-xs text-muted">
        {tier.description}
      </p>
      <ul className="mt-cluster flex flex-col gap-cluster-sm p-0">
        {tier.features.map((feature) => (
          <li
            className="flex items-center gap-cluster-sm text-footer text-ink"
            key={feature}
          >
            <span
              className={`size-1.5 shrink-0 rounded-full ${tier.featured ? "bg-[#0055ff]" : "bg-[#7d7594]"}`.trim()}
              aria-hidden="true"
            />
            {feature}
          </li>
      ...
      <div className="mt-auto grid gap-2 pt-5">
        ...
        {whatsappHref ? (
          <a
            className="inline-flex min-h-control-sm items-center justify-between gap-2 rounded-pill px-3.5 text-micro font-semibold text-[#006dce] ring-1 ring-[#b9ddff] transition-colors hover:bg-[#e8f3ff] focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-pink/35 focus-visible:outline-offset-3"
            href={whatsappHref}
            target="_blank"
            rel="noreferrer"
          >
...
export function ServicePricingSection({
  ...
  return (
    ...
    >
      ...
      <h2
        className="mt-cluster font-brand text-page-title text-ink max-lg:text-page-title-mobile"
    ...
    >
      <div className="grid gap-section-gap lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:gap-section-gap-xl">
        <div>
          ...
          <p className="mt-cluster max-w-[560px] text-body-lg text-muted">
            {faqDescription}
          </p>
          <p className="mt-section-gap-xl text-overline text-[#0055ff]">
            {faqSupportLabel}
          </p>
          <p className="mt-cluster-sm max-w-[460px] text-body-xs text-muted">
        ...
        <div className="border-y border-[#e0dee3]">
          {service.faqs.map((faq, index) => {
            ...
            return (
              <div
                className="border-b border-[#e0dee3] last:border-b-0"
                key={faq.question}
              >
                <button
                ...
                >
                  ...
                  <span
                    className={`inline-grid size-8 shrink-0 place-items-center rounded-full text-icon font-normal ${isOpen ? "bg-[#fce0e3] text-pink" : "bg-soft text-muted"}`.trim()}
                    aria-hidden="true"
                  >
                    <span
                      className={`transition-transform duration-200 ${isOpen ? "rotate-45" : ""}`.trim()}
                    >
                      +
                    </span>
                  </span>
                ...
                <div
                  className={`grid overflow-hidden text-body-xs text-muted transition-[grid-template-rows,padding] duration-[220ms] ${isOpen ? "grid-rows-[1fr] pb-4" : "grid-rows-[0fr]"}`.trim()}
                  id={answerId}
                  role="region"
                >
...
export function ServiceContactSection({
  ...
  return (
    <section
      className="mt-section-gap-xl flex flex-col gap-section-gap-lg rounded-nav bg-navy px-card-pad py-section-y text-white lg:mt-section-gap-xl lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-section-y-xl"
      id="service-contact"
    ...
    >
      <div>
        ...
        <h2
          className="mt-cluster max-w-[760px] font-brand text-page-title max-lg:text-page-title-mobile"
          id="service-contact-title"
        >
          {contactTitle}
        </h2>
        <p className="mt-cluster max-w-[680px] text-body-sm text-[#c7cfe0]">
      ...
      <div className="flex shrink-0 flex-col gap-2 sm:flex-row lg:flex-col xl:flex-row">
        <ContactModal
          ...
          variant="white"
          buttonClassName="min-h-button-lg min-w-[218px] justify-center text-body-xs"
          buttonLabel={contactButtonLabel}
        />
        {whatsappHref ? (
          <a
            className="inline-flex min-h-button-lg min-w-[218px] items-center justify-center gap-2 rounded-pill border border-white/35 px-4 text-button font-semibold text-white transition-colors hover:bg-white/10 focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-white/60 focus-visible:outline-offset-3"
            href={whatsappHref}
            target="_blank"
            rel="noreferrer"
          >
