...
function Field({ label, value, onChange, placeholder, className = "", type = "text", required = false, hint }: { label: string; value: string; onChange: (event: ChangeEvent<HTMLInputElement>) => void; placeholder?: string; className?: string; type?: string; required?: boolean; hint?: string }) {
  const id = useId();
  return (
...
function TextAreaField({ label, value, onChange, placeholder, className = "", required = false, hint }: { label: string; value: string; onChange: (event: ChangeEvent<HTMLTextAreaElement>) => void; placeholder?: string; className?: string; required?: boolean; hint?: string }) {
  const id = useId();
  return (
    <label className={`block min-w-0 ${className}`.trim()} htmlFor={id}>
      <span className={fieldLabelClass}>{label}{required ? <span className="ml-1 text-[#e44762]" aria-hidden="true">*</span> : null}</span>
...
function SectionDisclosure({ title, description, count, open = false, children }: { title: string; description: string; count?: number; open?: boolean; children: ReactNode }) {
  return (
    ...
    <div className="border-t border-[#e9e3da] pt-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {value ? <button className="min-h-9 rounded-full px-2.5 text-[11px] font-bold text-[#b34b60] transition-colors hover:bg-[#fff1f3]" type="button" onClick={() => onChange(null)} disabled={disabled}>Remove</button> : null}
        </div>
      </div>
      <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-[64px_minmax(0,1fr)] sm:items-center">
        ...
        <label className="min-w-0 flex-1">
          ...
          <select className={fieldClass} value={value ?? ""} disabled={disabled || loading} onChange={(event) => onChange(event.target.value || null)}>
      ...
      <p className="mt-2 text-[10px] leading-[1.45] text-[#9a938a]">The public page uses a cache-safe <code className="font-mono text-[#77736e]">/api/media/…</code> URL. An external URL remains available as an optional fallback.</p>
    </div>
  );
}
...
function DetailEditor({ detail, onChange, mediaAssetId, onMediaAssetChange }: { detail: ServiceDetailContent; onChange: (next: ServiceDetailContent) => void; mediaAssetId: string | null | undefined; onMediaAssetChange: (value: string | null) => void }) {
  ...
  return (
    <div className="overflow-hidden border-y border-[#e4ded5] bg-white/35">
      <SectionDisclosure title="Hero and media" description="CTA, quick details and the visual shown beside the service intro.">
        <div className="space-y-5">
          <div>
            <div className="mb-3">
              <p className={fieldLabelClass}>Quick details</p>
              <p className="mt-1 text-[11px] text-[#948d84]">Short facts customers see before reading the service content.</p>
            </div>
            <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
              <Field label="CTA label" value={detail.ctaLabel} onChange={(event) => update("ctaLabel", event.target.value)} placeholder="Talk to an advisor" />
              <Field label="Starting price" value={detail.startingPrice} onChange={(event) => update("startingPrice", event.target.value)} placeholder="From BDT 5,000" />
              <Field label="Delivery time" value={detail.deliveryTime} onChange={(event) => update("deliveryTime", event.target.value)} placeholder="2 to 3 days" />
              <Field label="Service mode" value={detail.serviceMode} onChange={(event) => update("serviceMode", event.target.value)} placeholder="Online or offline" />
            </div>
          </div>
          <div className="border-t border-[#e9e3da] pt-5">
            <div className="mb-3">
              <p className={fieldLabelClass}>Visual media</p>
              <p className="mt-1 text-[11px] text-[#948d84]">A managed image is preferred; the text remains accessible for every visitor.</p>
            </div>
            <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
              ...
              <Field className="sm:col-span-2" label="External media URL" value={detail.mediaUrl} onChange={(event) => update("mediaUrl", event.target.value)} placeholder="https://…" hint="Optional fallback when no managed image is selected." />
      ...
      <SectionDisclosure title="Overview and guided filing" description="The public-page body, managed through one rich-text editor." open>
        <div>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className={fieldLabelClass}>Public page body</p>
              <p className="mt-1 text-[11px] text-[#948d84]">Use headings, lists, links and images to explain the full service journey.</p>
            </div>
            <span className="rounded-full bg-[#f3f0eb] px-2.5 py-1 text-[10px] font-bold text-[#777168]">Rich text</span>
          </div>
          <div className="mt-4">
      ...
      <SectionDisclosure title="Helpful tools" description="Attach calculators or document builders that support this service." count={selectedTools.length}>
        ...
        <div className="mt-5 border-t border-[#e9e3da] pt-5">
          <label className="block min-w-0">
            ...
            <select className={`${fieldClass} mt-1.5`} value="" onChange={(event) => addTool(event.target.value)}>
          ...
          {selectedTools.length ? <div className="mt-4 divide-y divide-[#e9e3da] border-y border-[#e9e3da]">
            {selectedTools.map((slug) => {
              const tool = businessTools.find((candidate) => candidate.slug === slug);
              if (!tool) return null;
              return <div className="flex min-w-0 items-center justify-between gap-3 py-3" key={slug}>
          ...
          </div> : <p className="mt-3 text-[11px] text-[#948d84]">No tools attached. This section stays hidden on the public page until you add one.</p>}
        </div>
      </SectionDisclosure>

      <SectionDisclosure title="Key facts" description="Optional short facts shown beside the service overview on larger screens." count={detail.facts.length}>
        <div>
          ...
          <div className="mt-5 flex items-start justify-between gap-3 border-t border-[#e9e3da] pt-5"><div><p className={fieldLabelClass}>Facts shown beside the overview</p><p className="mt-1 text-[11px] text-[#948d84]">Leave this list empty and the public facts column stays hidden.</p></div><AddButton onClick={addFact}>+ Add fact</AddButton></div>
          {detail.facts.length ? <div className="mt-3 divide-y divide-[#e9e3da] border-y border-[#e9e3da]">{detail.facts.map((fact, index) => <div className="grid gap-2 py-3 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)_32px] sm:items-center" key={`fact-${index}`}><div className="min-w-0"><label className="sr-only" htmlFor={`service-fact-label-${index}`}>Fact {index + 1} label</label><input className={fieldClass} id={`service-fact-label-${index}`} value={fact.label} placeholder="Label" onChange={(event) => update("facts", detail.facts.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item))} /></div><div className="min-w-0"><label className="sr-only" htmlFor={`service-fact-value-${index}`}>Fact {index + 1} value</label><input className={fieldClass} id={`service-fact-value-${index}`} value={fact.value} placeholder="Value" onChange={(event) => update("facts", detail.facts.map((item, itemIndex) => itemIndex === index ? { ...item, value: event.target.value } : item))} /></div><RemoveButton label={`Remove fact ${index + 1}`} onClick={() => update("facts", detail.facts.filter((_, itemIndex) => itemIndex !== index))} /></div>)}</div> : <p className="mt-3 text-[11px] text-[#948d84]">No key facts added. The facts column will not render publicly.</p>}
        </div>
      </SectionDisclosure>

      <SectionDisclosure title="Pricing and booking" description="Optional packages, actions and WhatsApp prompts for this service." count={detail.pricing.length}>
        <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
        ...
        <div className="mt-5 border-t border-[#e9e3da] pt-5">
        <p className="max-w-[680px] text-[11px] leading-[1.5] text-[#948d84]">Add up to six packages. Each package receives a booking button; WhatsApp appears automatically when Contact settings has a number.</p>
        {detail.pricing.length ? <div className="mt-3 divide-y divide-[#e9e3da] border-y border-[#e9e3da]">
          {detail.pricing.map((tier, index) => <div className="relative py-4" key={`pricing-${index}`}>
            <div className="flex items-center justify-between gap-3"><p className={fieldLabelClass}>Package {String(index + 1).padStart(2, "0")}</p><RemoveButton label={`Remove package ${index + 1}`} onClick={() => update("pricing", detail.pricing.filter((_, itemIndex) => itemIndex !== index))} /></div>
      ...
      <SectionDisclosure title="FAQs" description="Answers that help customers decide before they contact your team." count={detail.faqs.length}>
        <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
        ...
        <div className="mt-5 border-t border-[#e9e3da] pt-5">
          {detail.faqs.length ? <div className="divide-y divide-[#e9e3da] border-y border-[#e9e3da]">{detail.faqs.map((faq, index) => <div className="relative py-4" key={`faq-${index}`}><div className="flex items-center justify-between gap-3"><p className={fieldLabelClass}>Question {String(index + 1).padStart(2, "0")}</p><RemoveButton label={`Remove FAQ ${index + 1}`} onClick={() => update("faqs", detail.faqs.filter((_, itemIndex) => itemIndex !== index))} /></div><div className="mt-3 grid gap-5"><Field label="Question" value={faq.question} onChange={(event) => update("faqs", detail.faqs.map((item, itemIndex) => itemIndex === index ? { ...item, question: event.target.value } : item))} /><TextAreaField label="Answer" value={faq.answer} onChange={(event) => update("faqs", detail.faqs.map((item, itemIndex) => itemIndex === index ? { ...item, answer: event.target.value } : item))} /></div></div>)}</div> : <p className="text-[11px] text-[#948d84]">No FAQs yet. The FAQ section stays hidden.</p>}
          <div className="mt-3"><AddButton onClick={addFaq}>+ Add FAQ</AddButton></div>
        </div>
      </SectionDisclosure>

      <SectionDisclosure title="Contact and booking" description="The final prompt that opens the shared contact form for this service.">
        <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
        ...
        <p className="mt-4 border-t border-[#e9e3da] pt-4 text-[11px] leading-[1.5] text-[#948d84]">The button opens the shared contact form. WhatsApp uses the global contact details configured in Contact settings.</p>
      </SectionDisclosure>
    </div>
  );
}
...
function ServiceEditor({ service, menuOptions, onSaved }: { service: AdminService; menuOptions: AdminServiceMenuOption[]; onSaved: (next: AdminService) => void }) {
  ...
  return (
    <section className="min-w-0 bg-white/60" aria-labelledby="service-editor-title">
      ...
      {message ? <p className="mx-4 mt-4 rounded-[12px] bg-[#effaf3] px-3.5 py-2.5 text-[12px] font-semibold text-[#29634d] sm:mx-6" role="status">{message}</p> : null}
      {error ? <p className="mx-4 mt-4 rounded-[12px] bg-[#fff4f5] px-3.5 py-2.5 text-[12px] font-semibold text-[#ad3148] sm:mx-6" role="alert">{error}</p> : null}

      <div className="space-y-8 p-4 sm:p-6 lg:p-7">
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          <section className="min-w-0 border-t border-[#e9e3da] pt-5" aria-labelledby="service-identity-title">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className={fieldLabelClass} id="service-identity-title">Page identity</p>
                <p className="mt-1 text-[12px] text-[#948d84]">Only the English title and public URL are required.</p>
              </div>
              <span className="text-[10px] font-medium text-[#9b958c]">Revision {draft.revision}</span>
            </div>
            <div className="mt-5 grid gap-x-4 gap-y-5 sm:grid-cols-2">
              ...
              <Field label="Public URL slug" value={draft.slug} required onChange={(event) => updateSlug(slugifyDraft(event.target.value))} placeholder="limited-company-registration" hint="Lowercase letters, numbers and hyphens only." />
            </div>
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#e9e3da] pt-4 text-[11px] text-[#7c756d]">
              ...
              <strong className="break-all font-mono text-[11px] font-semibold text-[#3f3c38]">/services/{draft.slug || "…"}</strong>
            </div>
            <details className="group mt-5 border-t border-[#e9e3da] pt-4">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-left [&::-webkit-details-marker]:hidden">
                <span><span className="block text-[12px] font-bold text-[#4f4b47]">Localization and card settings</span><span className="mt-1 block text-[10px] text-[#9b958c]">Bangla content, catalogue summaries and icon.</span></span>
                <span className="grid size-7 shrink-0 place-items-center rounded-full bg-[#f2efe9] text-[16px] font-normal text-[#847d74] transition-transform group-open:rotate-45" aria-hidden="true">+</span>
              </summary>
              <div className="mt-5 grid gap-x-4 gap-y-5 sm:grid-cols-2">
                ...
                <label className="block min-w-0"><span className={fieldLabelClass}>Service icon</span><div className="mt-1.5"><IconPicker value={draft.icon} onChange={(value) => update("icon", value)} disabled={saving} /></div></label>
          ...
          <section className="min-w-0 border-t border-[#e9e3da] pt-5" aria-labelledby="service-navigation-title">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className={fieldLabelClass} id="service-navigation-title">Menu connection</p>
                <p className="mt-1 text-[12px] text-[#948d84]">Connect this page to one live menu entry when it is ready.</p>
              </div>
              <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${service.assignedMenu ? "bg-[#e1f2e7] text-[#29634d]" : "bg-[#f2efe9] text-[#777168]"}`.trim()}>{service.assignedMenu ? "Connected" : "Not connected"}</span>
            </div>
            <label className="mt-5 block min-w-0">
              <span className={fieldLabelClass}>Choose menu entry</span>
              <select className={`${fieldClass} mt-1.5`} value={selectedMenuKey} disabled={saving || !menuOptions.length} onChange={(event) => setSelectedMenuKey(event.target.value)}><option value="">Not assigned</option>{menuGroups.map(([sectionLabel, options]) => <optgroup label={sectionLabel} key={sectionLabel}>{options.map((option) => { const inUse = Boolean(option.assignedProfileId && option.assignedProfileId !== service.profileId); return <option key={`${option.targetType}:${option.id}`} value={`${option.targetType}:${option.id}`} disabled={inUse}>{option.pathLabel}{inUse ? " · already assigned" : ""}</option>; })}</optgroup>)}</select>
            </label>
        ...
        <section className="border-t border-[#e9e3da] pt-6" aria-labelledby="service-content-title">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className={fieldLabelClass} id="service-content-title">Service page content</p>
              <p className="mt-1 text-[12px] text-[#948d84]">Work through only the public sections this service needs.</p>
            </div>
            {service.detail || detailOpen ? <span className="inline-flex min-h-8 items-center rounded-full bg-[#e4f3e8] px-3 text-[10px] font-bold text-[#29634d]">Content enabled</span> : <button className="inline-flex min-h-9 items-center rounded-full bg-[#f2efe9] px-3 text-[11px] font-bold text-[#5e5952] transition-colors hover:bg-[#ebe6de]" type="button" onClick={() => { setDetailOpen(true); setDraft((current) => ({ ...current, href: `/services/${current.slug}`, detail: current.detail ?? emptyDetail() })); }}>Add page content</button>}
          </div>
          {detailOpen ? <div className="mt-5"><DetailEditor detail={detail ?? emptyDetail()} mediaAssetId={draft.mediaAssetId} onMediaAssetChange={(value) => update("mediaAssetId", value)} onChange={(next) => update("detail", next)} /></div> : <p className="mt-5 border-t border-[#e9e3da] pt-4 text-[12px] text-[#817a72]">Save the title and URL first, then add the detail sections when you are ready.</p>}
        </section>
      </div>
    </section>
  );
