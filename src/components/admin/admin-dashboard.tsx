...
function StatCard({ value, label, detail, accent }: { value: string; label: string; detail: string; accent: string }) {
  return (
    <article className="rounded-[22px] border border-[#e1dcd4] bg-white p-5 sm:p-6">
      ...
      <strong className="mt-6 block font-brand text-[34px] font-bold leading-none tracking-[-0.05em] text-[#14131c]">{value}</strong>
...
export function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[24px] bg-[#14131c] p-5 text-white sm:p-7">
      ...
      <section aria-labelledby="workspace-stats-title">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#e44762]">Overview</p>
            <h2 className="mt-2 font-brand text-[25px] font-bold tracking-[-0.04em] text-[#14131c]" id="workspace-stats-title">Menu at a glance</h2>
          </div>
          <span className="hidden text-[12px] text-[#8b857e] sm:block">{loading ? "Syncing…" : "Synced with public navigation"}</span>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard value={loading ? "—" : String(stats.sections)} label="Sections" detail="Primary nav" accent="bg-[#fce0e3] text-[#e44762]" />
          <StatCard value={loading ? "—" : String(stats.groups)} label="Categories" detail="Menu groups" accent="bg-[#d7f0e3] text-[#29634d]" />
          <StatCard value={loading ? "—" : String(stats.items)} label="Services" detail="Public entries" accent="bg-[#e7e0f8] text-[#5c4aa6]" />
          <StatCard value={loading ? "—" : String(stats.links)} label="Sub-links" detail="Nested links" accent="bg-[#fae5cc] text-[#9e5726]" />
        </div>
      </section>
