    ...
    <main className="relative min-h-screen overflow-hidden bg-[#14131c] px-4 py-5 text-white sm:px-8 sm:py-8">
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-40px)] w-full max-w-[1180px] items-center gap-10 lg:grid-cols-[1fr_430px] lg:gap-20">
        <section className="hidden max-w-[560px] lg:block">
          <span className="inline-flex rounded-full border border-white/15 bg-white/8 px-3.5 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/65">Secure workspace</span>
          <h1 className="mt-6 max-w-[520px] font-brand text-[56px] font-bold leading-[0.98] tracking-[-0.05em] text-white xl:text-[70px]">Make every menu change feel simple.</h1>
          <p className="mt-6 max-w-[460px] text-[17px] leading-[1.6] text-white/60">Shape the public Limex experience from one calm workspace. Update sections, service links and category icons without touching the front-end code.</p>
          <div className="mt-10 grid max-w-[470px] grid-cols-3 gap-3">
            {[
              ...
              <div className="rounded-[18px] border border-white/10 bg-white/6 p-4" key={label}>
                <strong className="block text-[24px] tracking-[-0.04em] text-white">{value}</strong>
                <span className="mt-1 block text-[11px] leading-[1.35] text-white/40">{label}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto w-full max-w-[430px]">
          <div className="rounded-[28px] border border-white/10 bg-[#fcfbf9] p-5 text-[#14131c] shadow-[0_24px_80px_rgba(0,0,0,0.22)] sm:p-8">
            ...
            <p className="mt-3 text-[14px] leading-[1.55] text-[#77736e]">Sign in to manage the services your customers see across Limex.</p>
            ...
            <p className="mt-6 text-center text-[11px] leading-[1.45] text-[#aaa49b]">Access is controlled by the admin credentials configured in your environment.</p>
          </div>
          <a className="mx-auto mt-5 block w-max text-[12px] font-semibold text-white/45 transition-colors hover:text-white" href="/">← Back to Limex website</a>
        </section>
      </div>
    </main>
  );
