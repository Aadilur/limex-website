...
const inputClass = "mt-2 min-h-11 w-full rounded-[13px] border border-[#d9d3ca] bg-white px-3.5 text-[13px] text-[#14131c] outline-none transition-colors placeholder:text-[#aaa49b] focus:border-[#e44762] focus:ring-4 focus:ring-[#f54763]/10";
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(280px,0.42fr)]">
        <div className="rounded-[22px] bg-white/70 p-4 ring-1 ring-[#ddd8cf]/80 sm:p-5">
          <div className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#77736e]">Reach the team</p><h2 className="mt-1.5 font-brand text-[24px] font-bold tracking-[-0.04em] text-[#14131c]">Public contact details</h2></div><span className="grid size-10 place-items-center rounded-[12px] bg-[#e9f4ed] text-[16px] text-[#29634d]" aria-hidden="true">↗</span></div>
        ...
        <aside className="rounded-[22px] bg-[#14131c] p-5 text-white sm:p-6" aria-labelledby="contact-preview-title">
          ...
          <div className="mt-6 border-t border-white/10 pt-4"><p className="text-[11px] leading-[1.55] text-white/55">Use an international WhatsApp number. A safe wa.me link is generated only when the number is valid; otherwise no broken WhatsApp button is shown.</p>{settings?.whatsappUrl ? <a className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-full bg-white px-3.5 text-[12px] font-bold text-[#14131c]" href={settings.whatsappUrl} target="_blank" rel="noreferrer">Test WhatsApp link <span aria-hidden="true">↗</span></a> : <p className="mt-4 text-[11px] font-semibold text-[#f8bec8]">Add a number to enable WhatsApp actions.</p>}</div>
        </aside>
      </section>

      <section className="rounded-[22px] bg-white/70 p-4 ring-1 ring-[#ddd8cf]/80 sm:p-5">
        <div><p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#77736e]">Conversation defaults</p><h2 className="mt-1.5 font-brand text-[24px] font-bold tracking-[-0.04em] text-[#14131c]">WhatsApp message</h2><p className="mt-1 max-w-[620px] text-[12px] leading-[1.5] text-[#8b857e]">This message is prefilled when someone chooses “Discuss on WhatsApp” from a service or pricing tier.</p></div>
