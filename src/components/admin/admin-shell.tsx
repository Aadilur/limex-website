...
function LimexMark() {
  return <span className="font-brand text-[22px] font-bold tracking-[0.02em] text-white">LIMEX</span>;
}
  ...
  return (
    <>
      <div className="flex items-center justify-between border-b border-white/10 pb-6">
        ...
        <span className="grid size-9 place-items-center rounded-full bg-[#d7f0e3] text-[12px] font-bold text-[#29634d]">A</span>
      </div>
        ...
        <nav className="mt-3 space-y-1" aria-label="Admin navigation">
          {adminNavigation.map((item) => {
            ...
            return (
              <a
                key={item.href}
                className={`group flex items-center gap-3 rounded-[16px] px-3 py-3 transition-colors ${active ? "bg-white text-[#14131c]" : "text-white/65 hover:bg-white/10 hover:text-white"}`.trim()}
                href={item.href}
                onClick={onNavigate}
              >
      ...
      <div className="mt-auto border-t border-white/10 pt-5">
        <div className="flex items-center gap-3 rounded-[16px] bg-white/[0.08] p-3.5">
          <span className="grid size-8 shrink-0 place-items-center rounded-full bg-[#d7f0e3] text-[11px] font-bold text-[#29634d]">A</span>
...
export function AdminShell({ children }: { children: ReactNode }) {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f3f1ec] px-5">
        <div className="flex items-center gap-3 rounded-full border border-[#e4dfd7] bg-white px-5 py-3 text-[13px] font-semibold text-[#6c6863]">
          <span className="size-2 animate-pulse rounded-full bg-[#f54763]" aria-hidden="true" />
  ...
  return (
    <div className="min-h-screen bg-[#f3f1ec] text-[#14131c]">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[276px] flex-col bg-[#14131c] px-5 py-6 lg:flex">
        <SidebarContent />
        <button
        ...
        <header className="sticky top-0 z-30 border-b border-[#e4dfd7]/90 bg-[#f3f1ec]/90 px-4 py-4 backdrop-blur-[16px] sm:px-6 lg:px-10 lg:py-5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <button
                className="grid size-10 place-items-center rounded-full bg-[#14131c] text-white lg:hidden"
                type="button"
              ...
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#e44762]">Limex</p>
                <p className="mt-1 text-[15px] font-semibold text-[#14131c]">Admin</p>
              </div>
            </div>
            <div className="hidden items-center gap-3 sm:flex">
              <a className="rounded-full border border-[#d8d2c8] bg-white px-4 py-2.5 text-[12px] font-semibold text-[#4f4b47] transition-colors hover:border-[#bdb5aa] hover:text-[#14131c]" href="/" target="_blank" rel="noreferrer">
                View website ↗
              </a>
              <span className="grid size-10 place-items-center rounded-full bg-[#d7f0e3] text-[12px] font-bold text-[#29634d]">TA</span>
            </div>
          </div>
        </header>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button className="absolute inset-0 h-full w-full border-0 bg-[#14131c]/55" type="button" aria-label="Close admin navigation" onClick={() => setMobileOpen(false)} />
          <aside className="absolute bottom-3 left-3 top-3 flex w-[min(330px,calc(100%-24px))] flex-col rounded-[24px] bg-[#14131c] p-5 shadow-[0_20px_60px_rgba(20,20,28,0.28)]">
