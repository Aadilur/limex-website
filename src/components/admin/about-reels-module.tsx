  ...
  return (
    <form className="rounded-[22px] border border-[#d9d1c5] bg-white p-4 shadow-[0_14px_36px_rgba(49,42,35,0.06)] sm:p-5" onSubmit={handleSubmit}>
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#eee9e2] pb-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#e44762]">{reel ? "Edit reel" : "New reel"}</p>
          <h3 className="mt-1 font-brand text-[24px] font-bold tracking-[-0.04em] text-[#14131c]">Add a Limex story</h3>
        </div>
        <button className="grid size-9 place-items-center rounded-full border border-[#ddd7ce] text-[18px] text-[#77736e] transition-colors hover:border-[#aaa197] hover:text-[#14131c]" type="button" aria-label="Close reel editor" onClick={onCancel}>×</button>
      </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block min-w-0 sm:col-span-2">
            ...
            <input className="mt-2 min-h-11 w-full rounded-[12px] border border-[#ddd7ce] bg-[#fffdfa] px-3.5 text-[13px] text-[#14131c] outline-none transition-colors placeholder:text-[#aaa49b] focus:border-[#e44762] focus:ring-4 focus:ring-[#f54763]/10" id="about-reel-title" value={draft.title} placeholder="Leave blank to use the YouTube title" maxLength={160} onChange={(event) => updateDraft("title", event.target.value)} />
      ...
      <div className="mt-5 flex flex-wrap justify-end gap-2 border-t border-[#eee9e2] pt-4">
        ...
        <button className="inline-flex min-h-10 items-center justify-center rounded-full bg-[#14131c] px-5 text-[12px] font-bold text-white transition-transform hover:-translate-y-px disabled:cursor-wait disabled:opacity-60" type="submit" disabled={saving}>{saving ? "Saving…" : reel ? "Save reel" : "Add reel"}</button>
      </div>
    </form>
  );
}
    ...
    <article className="overflow-hidden rounded-[20px] border border-[#e1dcd4] bg-white transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_32px_rgba(49,42,35,0.06)]">
      <div className="flex gap-4 p-4 sm:p-5">
        <div className="relative aspect-video w-[126px] shrink-0 overflow-hidden rounded-[14px] border border-[#e8e1d8] bg-[#faf9f6] sm:w-[160px]">
          ...
          <span className="absolute bottom-2 left-2 rounded-full bg-[#14131c]/80 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.12em] text-white">YouTube</span>
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate text-[10px] font-bold uppercase tracking-[0.12em] text-[#e44762]">{reel.title ? "Custom title" : "YouTube title"}</p>
              <h3 className="mt-1 line-clamp-2 font-brand text-[20px] font-bold leading-[1.08] tracking-[-0.04em] text-[#14131c]">{displayTitle(reel)}</h3>
            </div>
          ...
          <a className="mt-2 block truncate text-[11px] text-[#8b857e] underline decoration-[#d8d2c8] underline-offset-2 transition-colors hover:text-[#14131c]" href={reel.youtubeUrl} target="_blank" rel="noreferrer">{reel.youtubeUrl}</a>
        </div>
      </div>
  ...
  return (
    <section className="border-t border-[#ddd7ce] pt-8" aria-labelledby="about-reels-admin-title">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#e44762]">Video reels</p>
          <div className="mt-2 flex items-center gap-2">
            <h2 className="font-brand text-[27px] font-bold tracking-[-0.04em] text-[#14131c]" id="about-reels-admin-title">Stories from Limex</h2>
        ...
        <button className="inline-flex min-h-10 items-center justify-center rounded-full bg-[#14131c] px-4 text-[12px] font-bold text-white transition-transform hover:-translate-y-px disabled:opacity-60" type="button" disabled={loading || saving} onClick={() => { setError(""); setNotice(""); setEditingId("new"); }}>+ Add reel</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-5 rounded-[20px] border border-dashed border-[#d8d1c7] bg-white px-5 py-12 text-center">
          <p className="font-brand text-[22px] font-bold tracking-[-0.03em] text-[#14131c]">Make the story visible.</p>
          ...
          <button className="mt-5 inline-flex min-h-10 items-center rounded-full bg-[#14131c] px-4 text-[12px] font-bold text-white" type="button" onClick={() => setEditingId("new")}>Add first reel</button>
        </div>
      )}
    </section>
  );
