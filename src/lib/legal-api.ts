...
export async function getPublicLegalPage(
  ...
  try {
    return await request<LegalPageData>(`/api/legal/${slug}`, {
      cache: "no-store",
    });
  } catch {
...
export async function getAdminLegalPages(): Promise<LegalPageData[]> {
  try {
    return await request<LegalPageData[]>("/api/admin/legal", {
      cache: "no-store",
    });
  } catch {
...
export async function getAdminLegalPage(
  ...
  try {
    return await request<LegalPageData>(`/api/admin/legal/${slug}`, {
      cache: "no-store",
    });
  } catch {
    return defaultLegalPages[slug];
