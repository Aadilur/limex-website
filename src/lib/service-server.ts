      ...
      : normalized.startsWith("/business-tools/")
        ...
        : normalized === "#contact" || normalized.startsWith("#contact-")
          ...
          : /^https?:\/\//i.test(normalized)
            ? "EXTERNAL"
            : "INTERNAL";
  const label = type === "DETAIL" ? "View service" : type === "BLOG" ? "Read guide" : type === "TOOL" ? "Open tool" : type === "EXTERNAL" ? "Open website" : type === "CONTACT" ? "Contact team" : "Open page";
...
function fallbackChildren(title: string) {
  for (const section of navigation) {
    for (const group of section.megaGroups ?? []) {
      ...
      if (item) return (item.children ?? []).map((child, index) => ({ id: `${title}-${index}`, label: child.label, href: child.href, isVisible: true, sortOrder: index }));
    }
  }
  return [];
}
...
function fallbackCategory(filter: string) {
  if (filter === "Trademark") return { label: "IP & Trademark", key: "ip-trademark" };
  if (filter === "Tax & compliance") return { label: "Compliance & Documentation", key: "compliance-documentation" };
  if (filter === "Business tools") return { label: "Business Tools", key: "business-tools" };
  return { label: "Startup & Licensing", key: "startup-licensing" };
}

function fallbackItem(service: (typeof services)[number], index: number): PublicService {
  ...
  const slug = service.href.match(/^\/services\/([^/?#]+)/i)?.[1] ?? `${category.key}-${service.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  return {
    id: `fallback-${index}`,
...
function fallbackDetail(): PublicServiceDetail {
  const base = fallbackItem(services.find((service) => service.title === trademarkRegistrationService.title) ?? services[0], 0);
  const detail: ServiceDetailContent = {
    ...
    contentLinkHref: trademarkRegistrationService.contentLinkHref ?? "#pricing",
  ...
  return {
    ...
    destination: fallbackDestination(`/services/${trademarkRegistrationService.slug}`),
    status: "PUBLISHED",
    hasDetailPage: true,
    detail,
  };
...
async function fetchBackend<T>(path: string): Promise<{ value: T | null; available: boolean; notFound: boolean }> {
  try {
    const response = await fetch(backendUrl(path), { cache: "no-store" });
    if (response.status === 404) return { value: null, available: true, notFound: true };
