import { resolveBlockFontSize, resolvePageSettings, resolveTemplateText, templateFontSizeMetrics, templatePaperDimensions, type DocumentTemplateDraft, type TemplateBlock, type TemplatePage, type TemplateValues } from "./document-templates";
...
function blockHtml(block: TemplateBlock, template: DocumentTemplateDraft, values: TemplateValues, defaultFontSize: ReturnType<typeof resolvePageSettings>["defaultFontSize"]) {
  ...
  if (block.type === "field") { const style = styleFor(block, defaultFontSize); return `<p style="${style}margin:0 0 12px;">${escapeHtml(values[block.fieldKey]?.trim() || "")}</p>`; }
