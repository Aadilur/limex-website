...
function blockHtml(block: TemplateBlock, template: DocumentTemplateDraft, values: TemplateValues, fields: TemplateField[], defaultFontSize: ReturnType<typeof resolvePageSettings>["defaultFontSize"]) {
  if (!isTemplateBlockVisible(block, values)) return "";
  if (block.type === "title" || block.type === "heading" || block.type === "paragraph") {
    const text = escapeHtml(resolveTemplateText(block.text, values, fields, false));
    if (!text.trim()) return "";
...
function pageBlocksHtml(blocks: TemplateBlock[], template: DocumentTemplateDraft, values: TemplateValues, defaultFontSize: ReturnType<typeof resolvePageSettings>["defaultFontSize"]) {
  const chunks: TemplateBlockInstance[][] = [];
  for (const block of blocks) {
    for (const instance of expandTemplateBlockInstances(template, block, values)) {
      ...
      else chunks.push([instance]);
    }
  }
  return chunks.map((chunk) => chunk.length > 1 ? `<div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));column-gap:24px;">${chunk.map((instance) => blockHtml(instance.block, template, instance.values, instance.fields, defaultFontSize)).join("")}</div>` : blockHtml(chunk[0].block, template, chunk[0].values, chunk[0].fields, defaultFontSize)).join("");
}
