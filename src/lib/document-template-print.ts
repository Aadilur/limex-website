...
function pageHtml(page: TemplatePage, index: number, template: DocumentTemplateDraft, values: TemplateValues, totalPages: number, pageWidth: string, pageHeight: string) {
  ...
  return `<section class="template-page" style="width:${pageWidth};min-height:${pageHeight};padding:${pagePadding};font-family:${fontFamily};">${page.blocks.map((block) => blockHtml(block, template, values)).join("")}${settings.showPageNumbers ? `<div class="page-number">${index + 1} / ${totalPages}</div>` : ""}</section>`;
}
