import { resolvePageSettings, resolveTemplateText, templateFontSizeMetrics, templatePaperDimensions, type DocumentTemplateDraft, type TemplateBlock, type TemplatePage, type TemplateValues } from "./document-templates";
...
function pageHtml(page: TemplatePage, index: number, template: DocumentTemplateDraft, values: TemplateValues, totalPages: number, pageWidth: string, pageHeight: string) {
  ...
  return `<section class="template-page" style="width:${pageWidth};height:${pageHeight};padding:${pagePadding};font-family:${fontFamily};">${pageBlocksHtml(page.blocks, template, values)}${settings.showPageNumbers ? `<div class="page-number">${index + 1} / ${totalPages}</div>` : ""}</section>`;
}
